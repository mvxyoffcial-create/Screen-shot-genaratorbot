"""
handlers/trim.py
Two-step trim flow.  Uses user's own watermark_text from settings.
"""
import logging
import time
...
async def trim_start_cb(client: Client, cb: CallbackQuery):
    ...
    trim_state[cb.from_user.id] = {"step": "start"}
    await cb.answer()
...
async def trim_input(client: Client, message: Message):
    ...
    ts = parse_time(message.text.strip())
    if ts is None:
        return await message.reply_text(
            "❌ Invalid time. Use <code>HH:MM:SS</code>, <code>MM:SS</code> or seconds."
        )

    if state["step"] == "start":
        trim_state[user.id] = {"step": "end", "start": ts}
        await message.reply_text(
            f"✅ <b>Start:</b> <code>{ts}</code>\n\n"
    ...
    status = await message.reply_text(
        f"✂️ <b>Trimming…</b>\n"
        f"From <code>{start_ts}</code> → <code>{end_ts}</code>\n"
